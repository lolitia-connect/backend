...
func (l *UpdateNodeGroupLogic) UpdateNodeGroup(req *types.UpdateNodeGroupRequest) error {
	...
	if req.IsExpiredGroup != nil && *req.IsExpiredGroup {
		count, err := l.svcCtx.Store.Group().CountExpiredNodeGroups(l.ctx, req.Id)
		if err != nil {
		...
		subscribeCount, err := l.svcCtx.Store.Subscribe().CountByDefaultNodeGroup(l.ctx, req.Id)
		if err != nil {
			zap.S().Errorf("failed to check subscribe usage: %v", err)
			return err
		}
		if subscribeCount > 0 {
	...
	if req.Type != "" {
		...
		nodeGroup.Type = nodeGroupType
	}
	if req.Sort != 0 {
		nodeGroup.Sort = req.Sort
	}
	if req.ForCalculation != nil {
		nodeGroup.ForCalculation = req.ForCalculation
	}
	if req.IsExpiredGroup != nil {
		nodeGroup.IsExpiredGroup = req.IsExpiredGroup
		// 过期节点组不参与分组计算
		if *req.IsExpiredGroup {
			forCalculation := false
			nodeGroup.ForCalculation = &forCalculation
		}
	}
	...
	if req.MaxTrafficGBExpired != nil {
		nodeGroup.MaxTrafficGBExpired = req.MaxTrafficGBExpired
	}
	...
	if req.MinTrafficGB != nil {
		...
		nodeGroup.MinTrafficGB = req.MinTrafficGB
	}
	if req.MaxTrafficGB != nil {
		newMaxTraffic = req.MaxTrafficGB
		nodeGroup.MaxTrafficGB = req.MaxTrafficGB
	}
	...
	// 执行更新
	if err := l.svcCtx.Store.Group().UpdateNodeGroup(l.ctx, nodeGroup); err != nil {
		zap.S().Errorf("failed to update node group: %v", err)
		return err
	}

	zap.S().Infof("updated node group: id=%d", req.Id)
	return nil
}
...
func (l *UpdateNodeGroupLogic) validateTrafficRange(currentNodeGroupId int, newMin, newMax *int64) error {
	...
	// 查询所有其他设置了流量区间的节点组
	otherGroups, err := l.svcCtx.Store.Group().QueryTrafficRangeNodeGroups(l.ctx, int64(currentNodeGroupId))
	if err != nil {
		zap.S().Errorf("failed to query other node groups: %v", err)
		return err
	}
