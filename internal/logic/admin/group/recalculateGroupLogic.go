...
func (l *RecalculateGroupLogic) RecalculateGroup(req *types.RecalculateGroupRequest) error {
	...
	now := time.Now()
	var history *ent.GroupHistory

	// 使用 Ent Transaction 执行分组重算
	tx, err := l.svcCtx.Ent.Tx(l.ctx)
	if err != nil {
		return err
	}
	err = func(tx *ent.Tx) error {
		// 创建历史记录
		history, err = tx.GroupHistory.Create().
			SetGroupMode(req.Mode).
		...
		// 更新状态为 running
		if err := tx.GroupHistory.UpdateOneID(history.ID).SetState("running").Exec(l.ctx); err != nil {
		...
		endTime := time.Now()
		if err := tx.GroupHistory.UpdateOneID(history.ID).
			SetState("completed").
			SetTotalUsers(affectedCount).
			SetSuccessCount(affectedCount).
			SetFailedCount(0).
			SetEndTime(endTime).
			Exec(l.ctx); err != nil {
	...
	if err != nil {
		...
		if history != nil {
			updateErr := l.svcCtx.Ent.GroupHistory.UpdateOneID(history.ID).
				SetState("failed").
...
func (l *RecalculateGroupLogic) getUserEmail(tx *ent.Tx, userId int64) string {
	authMethod, err := tx.UserAuthMethod.Query().
		Where(userauthmethod.UserID(userId), userauthmethod.AuthTypeIn("email", "6")).
		First(l.ctx)
	if err != nil {
		return ""
	}
...
func (l *RecalculateGroupLogic) countNodesByGroup(tx *ent.Tx, nodeGroupId int64) (int, error) {
	return tx.Node.Query().Where(predicate.Node(func(s *sql.Selector) {
...
func (l *RecalculateGroupLogic) executeAverageGrouping(tx *ent.Tx, historyId int64) (int, error) {
	// 1. 查询所有有效且未锁定的用户订阅（status IN (0, 1)）
	userSubscribes, err := tx.UserSubscribe.Query().
		Where(usersubscribe.GroupLocked(false), usersubscribe.StatusIn(0, 1)).
		All(l.ctx)
	if err != nil {
		return 0, err
	}
	...
	// 1.5 查询所有参与计算的节点组ID
	calculationNodeGroups, err := tx.NodeGroup.Query().Where(nodegroup.ForCalculation(true)).All(l.ctx)
	if err != nil {
	...
	subscribeInfos, err := tx.Subscribe.Query().Where(subscribe.IDIn(subscribeIds...)).All(l.ctx)
	if err != nil {
	...
	for _, us := range userSubscribes {
		...
		if len(subInfo.NodeGroupIds) > 0 {
			...
			if len(nodeGroupIds) == 0 && len(subInfo.NodeGroupIds) > 0 {
				l.Debugw("all node_group_ids are not for calculation, setting to 0",
					...
					logger.Field("total_node_groups", len(subInfo.NodeGroupIds)))
			}
		}
		...
		if len(nodeGroupIds) == 0 {
			...
			if err := tx.UserSubscribe.UpdateOneID(us.ID).SetNodeGroupID(0).Exec(l.ctx); err != nil {
		...
		if err := tx.UserSubscribe.UpdateOneID(us.ID).SetNodeGroupID(selectedNodeGroupId).Exec(l.ctx); err != nil {
			l.Errorw("failed to update user_subscribe node_group_id",
				logger.Field("user_subscribe_id", us.ID),
				logger.Field("error", err.Error()))
			failedCount++
	...
	for nodeGroupId, users := range groupUsersMap {
		...
		if nodeGroupId > 0 {
			var err error
			nodeCount, err = l.countNodesByGroup(tx, nodeGroupId)
			if err != nil {
		...
		if err := tx.GroupHistoryDetail.Create().
...
func (l *RecalculateGroupLogic) executeSubscribeGrouping(tx *ent.Tx, historyId int64) (int, error) {
	...
	expiredUserSubscribes, err := tx.UserSubscribe.Query().
		Where(usersubscribe.GroupLocked(false), usersubscribe.StatusNotIn(0, 1)).
		All(l.ctx)
	if err != nil {
		...
		for _, eu := range expiredUserSubscribes {
			...
			if err := tx.UserSubscribe.UpdateOneID(eu.ID).SetNodeGroupID(0).Exec(l.ctx); err != nil {
...
func (l *RecalculateGroupLogic) executeTrafficGrouping(tx *ent.Tx, historyId int64) (int, error) {
	...
	// 1. 获取所有设置了流量区间的节点组
	nodeGroups, err := tx.NodeGroup.Query().
		Where(nodegroup.ForCalculation(true), nodegroup.Or(nodegroup.MinTrafficGBGT(0), nodegroup.MaxTrafficGBGT(0))).
		All(l.ctx)
	if err != nil {
	...
	for _, us := range userSubscribes {
		...
		// 更新 user_subscribe 的 node_group_id 字段
		if err := tx.UserSubscribe.UpdateOneID(us.ID).SetNodeGroupID(targetNodeGroupId).Exec(l.ctx); err != nil {
