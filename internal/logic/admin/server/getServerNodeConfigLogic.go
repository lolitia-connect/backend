...
func (l *GetServerNodeConfigLogic) GetServerNodeConfig(req *types.GetServerNodeConfigRequest) (*types.GetServerNodeConfigResponse, error) {
	...
	if _, err := nodeStore.FindOneServer(l.ctx, req.ServerID); err != nil {
		l.Logger.Errorf("[GetServerNodeConfig] FindOneServer Error: %v", err.Error())
	...
	if err != nil {
		l.Logger.Errorf("[GetServerNodeConfig] FindServerConfigOverride Error: %v", err.Error())
	...
	if err := nodeconfig.ApplyOverride(&effective, override); err != nil {
		l.Logger.Errorf("[GetServerNodeConfig] ApplyOverride Error: %v", err.Error())
	...
	if err != nil {
		l.Logger.Errorf("[GetServerNodeConfig] OverrideResponse Error: %v", err.Error())
